...
class RemoveWindow:
	def __init__(self, desktopFile):
		...
		if package == "dpkg":
			warnDlg = gtk.Dialog(title="Menu Tuquito", parent=None, flags=0, buttons=(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
			warnDlg.add_button(gtk.STOCK_REMOVE, gtk.RESPONSE_OK)
			warnDlg.vbox.set_spacing(10)
			warnDlg.set_icon_from_file("/usr/share/tuquito/logo.png")
			labelSpc = gtk.Label(" ")
