...
class pluginclass( object ):
	...
	def __init__( self, MenuWin, toggleButton ):
		...
		self.gconfHandlers = []
		# Gconf stuff		
		self.gconf = EasyGConf( "/apps/tuquito-menu/plugins/applications/" )
	...
	def get_panel(self):
		...
		for applet in appletidlist:
			...
			if bonobo_id == "OAFIID:GNOME_tuquito-menu":
	...
	def checkMenuFolder( self ):
		if os.path.exists( os.path.join( os.path.expanduser( "~" ), ".tuquito", "tuquito-menu", "applications" ) ):
			return True
		try:
			os.makedirs( os.path.join( os.path.expanduser( "~" ), ".tuquito", "tuquito-menu", "applications" ) )
			return True
		except:
			pass
	...
	def add_to_panel(self, widget, desktopEntry):
		import random
		object_name = "tuquito-menu_"+''.join([random.choice('abcdefghijklmnopqrstuvwxyz0123456789') for x in xrange(8)])
	...
	def favoritesBuildSpace( self ):
		...
		self.MenuWin.SetPaneColors( [ space ] )
		space.show()
	...
	def favoritesBuildSeparator( self ):
		...
		self.MenuWin.SetPaneColors( [ separator ] )
		separator.show_all()
		return separator
	...
	def favoritesBuildLauncher( self, location ):
		try:
			...
			if favButton.appExec:
				...
				favButton.connect( "clicked", lambda w: self.MenuWin.hide() )
				
				self.MenuWin.setTooltip( favButton, favButton.getTooltip() )
	...
	def buildFavorites( self ):		
		try:
			from user import home			
			if (not os.path.exists(home + "/.tuquito/tuquito-menu/applications.list")):
				os.system("mkdir -p " + home + "/.tuquito/tuquito-menu/applications")
				os.system("cp /usr/lib/tuquito/tuquito-menu/applications.list " + home + "/.tuquito/tuquito-menu/applications.list")

			applicationsFile = open ( os.path.join( os.path.expanduser( "~" ), ".tuquito", "tuquito-menu", "applications.list" ), "r" )
	...
	def favoritesSave( self ):
		try:
			self.checkMenuFolder()
			appListFile = open( os.path.join( os.path.expanduser( "~"), ".tuquito", "tuquito-menu", "applications.list" ) , "w" )
			...
			msgDlg = gtk.MessageDialog( None, gtk.DIALOG_MODAL, gtk.MESSAGE_ERROR, gtk.BUTTONS_OK, _("Couldn't save favorites. Check if you have write access to ~/.tuquito/tuquito-menu")+"\n(" + e.__str__() + ")" )
			msgDlg.run();
			msgDlg.destroy();
	...
	def updateBoxes( self ):
		...
		if addedCategories:
			...
			for item in addedCategories:
				...
				self.MenuWin.setTooltip( item["button"], item["tooltip"] )
	...
	def buildCategoryList( self ):
		newCategoryList = [ { "name": _("All"), "icon": self.MenuWin.icon, "tooltip": _("Show all applications"), "filter":"", "index": 0 } ]
