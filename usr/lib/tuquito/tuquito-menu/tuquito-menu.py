...
try:
	...
	libc.call( "prctl", 15, "tuquito-menu", 0, 0, 0 )
	libc.close()
except ImportError:
	pass
...
ICON = "/usr/lib/tuquito/tuquito-menu/menu.png"
...
class MainWindow( object ):
	...
	def __init__( self, toggleButton ):
		...
		gladefile 	= os.path.join( self.path, "tuquito-menu.glade" )
		...
		plugindir = os.path.join( os.path.expanduser( "~" ), ".tuquito/tuquito-menu/plugins" )
	...
	def getSetGconfEntries( self ):
		...
		self.offset               = self.gconf.get( "int", "tuquito-menu_offset", 0 )
...
class MenuWin( object ):
	...
	def createPanelButton( self ):
		...
		if os.path.exists("/etc/tuquito/info"):
			import commands
			tooltip = commands.getoutput("cat /etc/tuquito/info | grep DESCRIPTION")
	...
	def showAboutDialog( self, uicomponent, verb ):
		...
		about.set_authors(["Mario Colque <mario@tuquito.org.ar>", "Clement Lefebvre <clem@linuxmint.com>", "Lars-Peter Clausen <lars@laprican.de>"])
		about.set_translator_credits(("translator-credits"))
		about.set_copyright(_("Based on USP from S.Chanderbally"))
		about.set_logo( gtk.gdk.pixbuf_new_from_file("/usr/lib/tuquito/tuquito-menu/logo.png"))
...
if len(sys.argv) == 2 and sys.argv[1] == "run-in-window":
	...
	gnomeapplet.bonobo_factory("OAFIID:GNOME_tuquito-menu_Factory",
                             gnomeapplet.Applet.__gtype__,
                             "tuquito-menu", "0", menu_factory)
